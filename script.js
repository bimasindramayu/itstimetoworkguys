...
const CONFIG = {
    DEBUG_MODE: true,
    APPS_SCRIPT_URL: 'https://script.google.com/macros/s/AKfycbzUZXE1lyD9OOknUwOGYo-P3x7oe4doQLHesUcqyec2SROg3WJcvpZKTd4Qhfr6WPFe/exec',
...
async function saveEdit() {
    ...
    if (statusSelect) {
        Logger.log('saveEdit', 'Status Value:', statusSelect.value);
    }
    Logger.log('saveEdit', 'Reason Textarea Found:', !!reasonTextarea);
    if (reasonTextarea) {
        Logger.log('saveEdit', 'Reason Value:', reasonTextarea.value);
    }
    
    // ===== VALIDATE REASON IF DITOLAK =====
    if (statusSelect && statusSelect.value === 'Ditolak') {
        Logger.log('saveEdit', 'Status is Ditolak, validating reason...');
    ...
    try {
        ...
        Logger.log('saveEdit', 'STEP 1: Collecting form data...');
        
        const inputs = document.querySelectorAll('.edit-input:not([readonly]), .edit-textarea, .edit-select');
        const updatedData = {};
        
        Logger.log('saveEdit', 'Found form inputs:', inputs.length);
        ...
        inputs.forEach((input, idx) => {
            ...
            if (field) {
                ...
                Logger.log('saveEdit', `  [${idx}] ${field} = "${value}"`);
            }
        });

        Logger.log('saveEdit', 'Total fields collected:', Object.keys(updatedData).length);
        
        // ===== STEP 2: HANDLE ALASAN DITOLAK =====
        Logger.log('saveEdit', 'STEP 2: Handling Alasan Ditolak...');
        
        if (updatedData['Status'] !== 'Ditolak') {
            Logger.log('saveEdit', 'Status not Ditolak, setting reason to "-"');
        ...
        // ===== STEP 3: UPDATE ALLDATA ARRAY =====
        Logger.log('saveEdit', 'STEP 3: Updating allData array...');
        ...
        if (originalIndex !== -1) {
            ...
            Logger.log('saveEdit', 'WARNING: Row not found in allData');
        }

        // ===== STEP 4: UPLOAD FILES IF ANY =====
        Logger.log('saveEdit', 'STEP 4: Checking for file uploads...');
        Logger.log('saveEdit', 'Files to upload:', Object.keys(filesToUpload).length);
        
        if (Object.keys(filesToUpload).length > 0) {
            Logger.log('saveEdit', 'FILES FOUND - Starting upload process...');
            hideLoading(true);
            showLoading(`Mengupload ${Object.keys(filesToUpload).length} file...`, true);
            
            const uploadResult = await uploadFiles();
            
            Logger.log('saveEdit', 'Upload result:', {
                success: uploadResult.success,
                message: uploadResult.message,
                fileLinksCount: Object.keys(uploadResult.fileLinks || {}).length
            });
            
            if (uploadResult.success) {
                Logger.log('saveEdit', 'Upload SUCCESS - Merging file links to updatedData');
                
                const fileLinks = uploadResult.fileLinks || {};
                Logger.log('saveEdit', 'File links to merge:', Object.keys(fileLinks));
                
                for (let key in fileLinks) {
                    Logger.log('saveEdit', `  Merging: ${key} = ${fileLinks[key]}`);
                    updatedData[key] = fileLinks[key];
                    currentRowData[key] = fileLinks[key];
                }
                
                Logger.log('saveEdit', 'Updated data now contains:', Object.keys(updatedData).length, 'fields');
                hideLoading(true);
                showAlert('✅ File berhasil diupload', 'success', true);
            } else {
                hideLoading(true);
                showAlert('❌ Gagal mengupload file: ' + uploadResult.message, 'error', true);
                Logger.log('saveEdit', 'ERROR: Upload failed');
                return;
            }
        } else {
            Logger.log('saveEdit', 'No files to upload - skipping upload step');
        }

        // ===== STEP 5: SEND ALL DATA TO SPREADSHEET =====
        hideLoading(true);
        showLoading('Memperbarui data di spreadsheet...', true);
        
        Logger.log('saveEdit', 'STEP 5: Sending data to Apps Script...');
        Logger.log('saveEdit', 'Row Index:', currentRowData.rowIndex);
        Logger.log('saveEdit', 'Updated Data Fields:', Object.keys(updatedData));
        ...
        Logger.log('saveEdit', 'Sending POST request...');
        Logger.log('saveEdit', 'URL:', APPS_SCRIPT_URL);
        Logger.log('saveEdit', 'Body size:', formData.toString().length, 'bytes');
        ...
        Logger.log('saveEdit', 'Response status:', response.status);
        ...
        if (result.success) {
            Logger.log('saveEdit', '========== SAVE SUCCESS ==========');
            ...
            Logger.log('saveEdit', 'Message:', result.message);
        ...
        Logger.log('saveEdit', 'EXCEPTION:', error.message);
        Logger.log('saveEdit', 'Stack:', error.stack);
...
async function uploadFiles() {
    Logger.log('uploadFiles', '========== START UPLOAD FILES ==========');
    
    try {
        const nomorPeserta = currentRowData['Nomor Peserta'];
        Logger.log('uploadFiles', 'Nomor Peserta:', nomorPeserta);
        Logger.log('uploadFiles', 'Files to upload:', Object.keys(filesToUpload));
        
        if (!nomorPeserta) {
            Logger.log('uploadFiles', 'ERROR: No nomor peserta');
            return { success: false, message: 'Nomor peserta tidak ditemukan', fileLinks: {} };
        }

        if (Object.keys(filesToUpload).length === 0) {
            Logger.log('uploadFiles', 'No files to upload');
            return { success: true, message: 'No files', fileLinks: {} };
        }
        ...
        Logger.log('uploadFiles', 'Processing files...');
        
        const filePromises = [];
        
        for (let field in filesToUpload) {
            const file = filesToUpload[field];
            Logger.log('uploadFiles', `Processing: ${field} (${file.name}, ${file.size} bytes)`);
                ...
                new Promise((resolve, reject) => {
                    ...
                    reader.onload = function(e) {
                        try {
                            ...
                            Logger.log('uploadFiles', `✓ Added to FormData: ${field}`);
                            resolve();
                        } catch (err) {
                    ...
                    reader.onerror = function(err) {
                        Logger.log('uploadFiles', `ERROR reading file: ${file.name}`);
        ...
        Logger.log('uploadFiles', 'All files processed');

        Logger.log('uploadFiles', 'Sending to Apps Script...');
        ...
        if (!response.ok) {
            Logger.log('uploadFiles', `HTTP Error: ${response.status}`);
            return { success: false, message: `HTTP ${response.status}`, fileLinks: {} };
        }

        const result = await response.json();
        
        Logger.log('uploadFiles', 'Server response received');
        Logger.log('uploadFiles', 'Full response:', result);
        Logger.log('uploadFiles', 'Response success:', result.success);
        Logger.log('uploadFiles', 'Response message:', result.message);
        Logger.log('uploadFiles', 'Response details:', result.details);

        if (result.success) {
            // CRITICAL: Extract fileLinks from result.details.fileLinks
            const fileLinks = (result.details && result.details.fileLinks) ? result.details.fileLinks : {};
            
            Logger.log('uploadFiles', 'File links extracted:', Object.keys(fileLinks).length);
            Logger.log('uploadFiles', 'File links object:', fileLinks);
            
            for (let key in fileLinks) {
                Logger.log('uploadFiles', `  ${key}: ${fileLinks[key]}`);
            }
            ...
            Logger.log('uploadFiles', 'Upload failed from server:', result.message);
            return { 
                success: false, 
                message: result.message || 'Gagal mengupload file',
                fileLinks: {} 
            };
        }

    } catch (error) {
        Logger.log('uploadFiles', 'EXCEPTION:', error.message);
        Logger.log('uploadFiles', 'Stack:', error.stack);
        return { 
            success: false, 
...
function handleFileUploadOnly(e) {
  try {
    ...
    return createResponse(true, 'File berhasil diupload', nomorPesertaOriginal, {
      ...
      fileLinks: fileLinks  // PENTING: Include file links dalam response
    });
...
function updateFileLinksInSheet(sheet, rowIndex, fileLinks) {
  try {
    ...
    Logger.log('File links keys: ' + Object.keys(fileLinks).join(', '));
    
    // Get all headers
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    Logger.log('Total headers in sheet: ' + headers.length);
    
    // Create detailed header map
    const headerMap = {};
    headers.forEach((header, idx) => {
      headerMap[header] = idx + 1; // Column index (1-based)
      if (header.includes('Link')) {
        Logger.log(`Header ${idx + 1}: "${header}"`);
      }
    });
    ...
    Logger.log('Total mappings to process: ' + Object.keys(linkMappings).length);
    ...
    for (let fileKey in fileLinks) {
      ...
      Logger.log('Processing file key: ' + fileKey);
      Logger.log('  Header name: ' + headerName);
      Logger.log('  URL: ' + url);
      ...
      if (!colIndex) {
        ...
        Logger.log(`  Available headers with "Link": `);
      ...
      // Update cell dengan URL
      try {
        const cell = sheet.getRange(rowIndex, colIndex);
        cell.setValue(url);
        Logger.log(`  ✓ Updated ${fileKey} at row ${rowIndex}, column ${colIndex}`);
        updatedCount++;
      } catch (cellError) {
        Logger.log(`  ERROR updating cell: ${cellError.message}`);
        skippedCount++;
      }
    }
...
function debugCheckLinkColumns() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName(SHEET_NAME);
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  
  Logger.log('=== DEBUG: ALL LINK COLUMNS ===');
  let linkCount = 0;
  for (let i = 0; i < headers.length; i++) {
    if (headers[i].includes('Link')) {
      Logger.log(`Column ${i + 1}: "${headers[i]}"`);
      linkCount++;
    }
  }
  Logger.log('Total link columns: ' + linkCount);
